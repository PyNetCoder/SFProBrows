...
This project is actively developed and maintained by **HfdSwimmingTimeHardwareAndSoftwareCorporation** to contribute to the open-source community.
...
1. Navigate to the latest release or build folder.
2. Launch `SFProBrows.exe` directly to start the browser.

## 📦 Deployment (For Developers)
Stable production releases for Windows users are compiled and packaged into a professional installation wizard (`.exe`) using **Inno Setup**. Future production-ready setups and installers will be available under the `Output/` directory.
